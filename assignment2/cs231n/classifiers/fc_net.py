...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        H = affine_relu_forward(X, self.params['W1'], self.params['b1'])
        S = affine_forward(H[0], self.params['W2'], self.params['b2'])
        scores = S[0]
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        ############################################################################
        smL, dS = softmax_loss(scores, y)
        rL = self.reg*.5*(np.sum(self.params['W1']**2) + np.sum(self.params['W2']**2))
        loss = smL + rL
        dH = affine_backward(dS, S[1])
        dX = affine_relu_backward(dH[0],H[1])
        grads['W1'] = dX[1] + self.reg*self.params['W1']
        grads['b1'] = dX[2]
        grads['W2'] = dH[1] + self.reg*self.params['W2']
        grads['b2'] = dH[2]
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        #print('y' + str(y))
        smL, dS = softmax_loss(scores, y)
        rL = np.zeros(self.num_layers)
        for i in range(self.num_layers):
            rL[i] = self.reg*.5*(np.sum(self.params['W'+str(i)]**2))
        loss = smL + rL.sum()
        #F[0][1] = dS
        lg = {}
        lg['W'+str(self.num_layers-1)] = affine_backward(dS, F[self.num_layers][1])
